...
int main(int argc, char** argv)
{
    ...
    int number_bytes = recvfrom(s, buffer, BUFFER_SIZE - 1, 0, (struct sockaddr *)&incoming_addr, &incoming_addr_size);
    ...
    if(strcmp(buffer, "ftp") == 0){
        ...
        number_bytes = sendto(s, "yes", 3, MSG_CONFIRM, (struct sockaddr *)&incoming_addr, incoming_addr_size);
    }else{
        // send information back to the client
        number_bytes = sendto(s, "no", 2, MSG_CONFIRM, (struct sockaddr *)&incoming_addr, incoming_addr_size);
    }
}
